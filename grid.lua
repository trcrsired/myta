local Myta = LibStub("AceAddon-3.0"):GetAddon("Myta",true)
if not Myta then return end
Myta.grid = {}
...
function Myta.CreateGrid(name,co,secure)
	local t =Myta.grid
	t[name] = co
...
function Myta.GridConfig(t,frame,background,center_text,bottom_text,cd,secure_frame)
...
function Myta.GridCenter(tb,count,L,M,center_text,format)
	if tb == nil then
		return
	end
	if count < L then
