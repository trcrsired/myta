...
function Myta.CreateBar(name,co)
	local t =Myta.bar
	t[name] = co
...
function Myta.BarConfig(t,frame,background,statusbar,percentage,amount)
	local tb = t.bar
	if tb == nil then
		tb = {}
		t.bar = tb
...
function Myta.BarSet(tb,current,max,statusbar,percentage,amount)
